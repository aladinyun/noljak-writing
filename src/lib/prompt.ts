...
${school
  ...
  : `- 인근 학교명을 임의로 지어내지 말 것`}`
      ...
      channelGuide = `- 채널: 맘카페 등 커뮤니티에 원장이 올리는 소개글
- 광고처럼 보이지 않는 자연스럽고 편안한 어조로 작성
- 원장 본인이 직접 작성하는 글이되, 광고처럼 딱딱하지 않고 학부모에게 편하게 말을 건네듯 친근하고 진솔한 어조로 작성할 것. '저희 교육원은', '저희 센터에서는' 등 원장 시점의 표현을 명확히 유지하고, 학부모가 쓴 후기글로 오인될 수 있는 표현(예: '다녀와봤는데', '보내보니')은 사용하지 말 것. 노골적인 홍보 문구는 피하되 화자는 반드시 원장으로 유지.
- "최고", "1등" 등 노골적인 홍보 문구는 사용하지 말 것
- 실제 수업·성장 사례를 원장 시점에서 구체적으로 서술할 것
- SEO 키워드 반복은 지양하고 자연스러운 흐름을 우선할 것
${loc ? `- 지역(${loc})은 자연스러운 맥락에서만 언급할 것` : `- 지역명을 임의로 지어내지 말 것`}`
